...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let handleA1 = thread::spawn(move || a.into_iter().sum::<i32>());
    let handleB1 = thread::spawn(move || b.into_iter().sum::<i32>());
    (handleA1.join().unwrap(), handleB1.join().unwrap())
}
...
pub fn handle_panic(value: i32, should_panic: bool) -> Result<i32, ()> {
    ...
    let h = thread::spawn(move || match should_panic {
        true => {
            panic!("oops")
        }
        false => value,
